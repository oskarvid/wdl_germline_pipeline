...
HASH=$1
for D in `find cromwell-executions/*/$HASH/ -mindepth 1 -maxdepth 1 -type d -printf "%Tx %.10TX %p\n" | sort | awk {'print $6'}`; do \
START=$(find $D -maxdepth 2 \( -name 'shard*' -o -name 'call*' \) -printf '%Tx at %.10TX\n' | sort | head -n 1) && \
echo "$(basename $D | cut -c 6-) started on $START" && \
STOPP=$(find $D -maxdepth 3 -type f \( -name "*.bam" -o -name "*.vcf" -o -name "stderr" -o -name "stdout" \) -printf 'And stopped on %Tx at %.10TX \n' | sort | tail -n 1) && \
echo $STOPP && \
